...
class Scaler_Splitter(Regression):
    def __init__(self):
        # X_train, X_test, z_train, z_test = train_test_split(self.X, z, test_size= test_size)
        print("Scaler: ", self.X[0])
        if self.method == "Manual":
            print("whatever MANUAL")
...
class Manual_OLS(Scaler_Splitter):
    ...
    def OLS_Regression(self, z, test_size):
        ...
        print(self.X.shape, z.shape)
        X_train, X_test, z_train, z_test = train_test_split(self.X, z, test_size= test_size)
        X_train_scaled = Regression.numpy_scaler(X_train)
        X_test_scaled = Regression.numpy_scaler(X_test)
        X_scaled = Regression.numpy_scaler(self.X)
        
        self.beta = self.find_beta(X_train_scaled, z_train)
        z_tilde = X_train_scaled @ self.beta
        z_pred = X_test_scaled @ self.beta
        z_all_set = X_scaled @ self.beta
        ...
        Fr = fr.Franke()
        x_array, y_array = Fr.initialize_array(random = True)
        X = Regression.create_design_matrix(x_array, y_array, deg = 3)
        X = Regression.numpy_scaler(X)
        Z = X @ self.beta
        x_array = np.random.choise(x_array, len())
        x, y = np.meshgrid(x_array, y_array)
        print(x_array.shape, y_array.shape, X.shape, self.X.shape, self.beta.shape, Z.shape, x.shape, y.shape)
        print(x[:,:20].shape, y[:,:20].shape)
        print(x_array, y_array)
        # print("X: ", x[:,:20], "Y: ", y[:,:20])
        # Fr.plot_franke(x[:,:20], y[:,:20], Z)
        Fr.contour_franke(x[:,:20], y[:,:20], Z)
        
        # X = Regression.create_design_matrix(self.x, self.y, deg = 3)
        # X = Regression.numpy_scaler(X)
        # Z = X @ self.beta
        ...
        # plt.title("New Z")
        # print(x[:end,:].shape, y[:end,:].shape, " and ", z_tilde.shape, " X_train: ", X_train.shape)
        # print(x[:end,:], " and ", X_train )
        # print("z_tilde: ", z_tilde)
        # x_tilde = x[:end,:]; y_tilde = y[:end,:]
        # Fr.contour_franke(x_tilde, y_tilde, z_tilde)
        # Fr.plot_franke(x_tilde, y_tilde, z_tilde)
        # plt.title("z_tilde")
        # Fr.plot_franke(x, y, z_all_set)
        # plt.title("z_all_set")
        """
